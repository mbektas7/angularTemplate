...
export class AuthService {
    ...
    login(loginUser: UserLogin) {
        ...
        return this.httpClient
            .post(environment.rootPath + "Token/", loginUser, {
                headers: headers,
            })
            .shareReplay();
    }
    ...
    async logOut() {
        await this.httpClient
            .get(environment.rootPath + `Token/logout`)
            .toPromise();
        this.saveToken(null);
    ...
    isUserLoggedIn() {
        return this.httpClient.post(
            environment.rootPath + `Token/isUserLoggedIn`,
            null
        );
    }
    deleteRefreshToken() {
    ...
    isUserLoggedInWithoutRefreshing() {
        return this.httpClient.post(
            environment.rootPath + `Token/isUserLoggedInWithoutRefreshing`,
            null
        );
    }
    getCurrentUserId() {
    ...
    refreshToken() {
        ...
        const req = this.httpClient
            .get(environment.rootPath + `Token/refresh`)
            .do((res) => {
                if (res) {
